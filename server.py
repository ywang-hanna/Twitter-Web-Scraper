...
def add_color(tweets):
    ...
    for t in tweets:
        score = t['score']
        score_id = round((score + 1) * 50)
        color = colors[score_id]
        t['color'] = color
...
def tweets(name):
    ...
    med = median([t['score'] for t in tweets['tweets']])
    return render_template("tweets.html", record = tweets, median = med)
...
def following(name):
    ...
    sort_friends = sorted(friends, key = lambda d: d['followers'], reverse = True)
    return render_template("following.html", screen_name = name, users = sort_friends)
